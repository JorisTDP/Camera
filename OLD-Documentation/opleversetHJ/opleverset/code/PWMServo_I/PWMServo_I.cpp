#include "Arduino.h"
#include "PWMServo_I.h"

/*
renamed and adapted (for teensy use) to PWMServo_I by G.A.A for invertted pulse output
*/
/*
...
uint8_t PWMServo_I::attachedA = 0;
uint8_t PWMServo_I::attachedB = 0;
#ifdef SERVO_PIN_C
uint8_t PWMServo_I::attachedC = 0;
#endif

// ------------------------------------------------------------------------------------------------
void PWMServo_I::seizeTimer1()
{
...
// ------------------------------------------------------------------------------------------------
void PWMServo_I::releaseTimer1() {}

PWMServo_I::PWMServo_I() : pin(0), angle(NO_ANGLE) {}

uint8_t PWMServo_I::attach(int pinArg, int min, int max)
{
  #ifdef SERVO_PIN_C
...
// ------------------------------------------------------------------------------------------------
void PWMServo_I::write(int angleArg)
{
  uint16_t p;
...
// ------------------------------------------------------------------------------------------------
// ----------------------------------------------------------------------------------------
uint32_t PWMServo_I::attachedpins[(NUM_DIGITAL_PINS + 31) / 32]; // 1 bit per digital pin

PWMServo_I::PWMServo_I() : pin(255), angle(NO_ANGLE) {}
...
//	Serial.printf("attach, pin=%d, min=%d, max=%d\n", pinArg, min, max);
	...
	min16 = min >> 4;	// dived by 16 towards a 8-bits value
	max16 = max >> 4;
	angle = NO_ANGLE;
...
//	Serial.printf("attach result, pin=%d, min16=%d, max16=%d\n", pin, min16, max16);
	return 1;
}
...
//	Serial.printf("write, pin=%d, angle=%d\n", pin, angleArg);
	...
	angle = angleArg;
/*	Serial.printf("start value %d; hoek waarde %d , /2048 %d\n", (min16 << 12), 
	((max16 - min16) * 46603 * angle), ((max16 - min16) * 46603 * angle) >> 11);	*/
...
/*	float usec = (float)((max16 - min16)<<4) * ((float)angle / 180.0f) + (float)(min16<<4);
	...
	analogWrite(pin, 4095 - duty);//invert pwm signal
}

// ------------------------------------------------------------------------------------------------
uint8_t PWMServo_I::attached()
{
